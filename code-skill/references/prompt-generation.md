...
13. Present `CODE READY` with Quick Check evidence, then launch an awaited Agent (`End Task-{concise related task name}`, `run_in_background: false`) and wait for its verdict before reporting done. In that Ending Agent, test the prompt with representative input/output scenarios. Use repeated fresh runs for stochastic production prompts: default 3 and 5 for critical image, structured-data, or reliability claims. Report artifact creation separately from semantic/file/visual acceptance, and reopen with a corrected prompt if validation fails.
