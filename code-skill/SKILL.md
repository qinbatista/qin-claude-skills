...
9. Start the scored lifecycle ledger and bind the producer receipt when present. End Task is hard-required. For code that needs verification, build a real-test plan with `verify-skill/scripts/ending_verification_plan.py`; launch one independently scored/modelled Agent (`run_in_background: false`, prompt starting `ENDING_TASK_WORKER`) with description exactly `End Task-{task}-{check}` per independent acceptance check, and await every one before claiming the code is done. Each task runs its actual command. All checks must PASS. A FAIL creates a separate `Fix Task-{task}-{check}` with the exact error, followed by a fresh Ending check, for up to three repair attempts. When the `Agent` tool is unavailable, return terminal `BLOCKED` plus the exact handoffs; never treat the lifecycle as verified, emulate task creation, or self-verify — the verifier is always a different agent/model from the producer.

The smallest safe syntax, existence, direct-reference, or focused local execution check belongs to producer Quick Check. Full builds, broad lint, integration/API calls, large-file processing, live side effects, and regressions belong to the awaited Ending Agent unless they are themselves the user's requested task.

The mandatory post-result Ending lifecycle still runs. Show published code after Quick Check, then run the real-test Ending tasks and wait for their verdicts; `CODE READY` is a progress presentation, not a done claim. A failing Ending verifier records exact evidence and launches a separate repair task; the repaired code gets a fresh different verifier. The lifecycle always records score/band, check evidence, selected verifier pair, and repair chain locally; a receipt-backed producer terminal event also records score, route direction, and next pair in Obsidian.
