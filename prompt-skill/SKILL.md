...
11. Run the mandatory scored Ending lifecycle as awaited Agent tasks (`run_in_background: false`) and wait for every verdict before reporting the prompt work done. When the prompt change needs behavioral verification, create separate real representative-case checks with `ending_verification_plan.py`, each using its own score-derived model/effort; all required cases must PASS.
