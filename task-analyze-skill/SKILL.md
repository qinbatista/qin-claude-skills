...
This is the full routing and model-strategy skill. The hookless bootstrap in `~/.claude/CLAUDE.md` combines the saved highest-generation quality ladder with matching project-scoped broad Obsidian `Claude Model Switch.md` records; explicit multi-node routing still requires activation. The catalog fast producer (haiku) is schedule-only for disjoint independent sources, never the ordinary Auto first attempt. The mandatory post-result Ending lifecycle always applies.
...
Every adaptive profile reads `assets/model-capability-ladder.json`. The file is the saved local-catalog snapshot, and only its highest generation Claude model participates in ordinary quality movement. Ordinary result producers execute the contextual quality pair directly. The catalog fast producer (haiku) is reserved for fixed disjoint-source schedule branches. Move effort before model while downgrading and reverse that order while upgrading.
...
Ending Real alone records receipt-backed producer pass/fail evidence to the canonical broad Obsidian `Claude Model Switch.md`; producers and ordinary runners never write learning. Operational failures remain neutral and inline execution never fabricates a receipt.
...
- One result producer uses its resolved Obsidian context through `scripts/obsidian_adaptive_model_runner.py` and `scripts/model_execution_receipt.py`; ordinary text/code work executes the recommended quality pair, then at most one stronger operational fallback, and emits the first completed result without a foreground verification gate.
...
- `obsidian_adaptive_model_runner.py` reads learning but never writes it. The schedule-only fast producer cannot replace an ordinary quality recommendation. `strategy_performance.py` separately gates multi-node strategy JSON routing and Auto-versus-Direct claims.
