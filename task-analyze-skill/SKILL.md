...
4. Immediately after presentation, write a scored lifecycle receipt and bind `--producer-receipt` when present. End Task is hard-required. If the result needs verification, build `verify-skill/scripts/ending_verification_plan.py plan` with exact observable checks. Create one independently scored/modelled `End Task-{task}-{check}` for each independent real test/API/build/render/state check (prompt starting `ENDING_TASK_WORKER`, description exactly that title); all checks must PASS before the result is reported done. When the Agent tool is callable, run those checks with `run_in_background: false` and await them. Otherwise — including a headless, worker, or benchmark execution surface — return BLOCKED handoffs; the outer host must create the End Tasks. Never inspect or invoke app-server internals or let the producer verify itself. A FAIL records exact command/output/error, creates `Fix Task-{task}-{check}`, and after repair creates a fresh Ending check, for up to three attempts. Missing task creation, external infrastructure, or exhausted repairs is terminal BLOCKED and never verified. The origin waits for every check and reports the verdict in the same turn. Local history stores score/check/model/evidence/repair chain; receipt-backed producer terminal events also write score, Real verdict, and switch movement to Obsidian.
...
6. Every origin result visibly reports `Complexity: N/100 (band)` plus `Route change: downgrade|upgrade|freeze|no_switch|operational_fallback`. The origin final is complete only after the End Task reports; result presentation alone does not end the turn. The End Task final requires lifecycle `PASS` or explicit `BLOCKED`; concurrent or missing evidence is terminal BLOCKED, not a user question. A repair is never automatic: it requires a new user request after a concrete recorded failure.
...
4. Prove ordinary code tasks run only one proportional producer Quick Check, present immediately afterward, create a separately titled End Task, and await its verdict before reporting done rather than letting the producer verify itself.
