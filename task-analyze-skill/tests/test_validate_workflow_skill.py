...
class ValidateWorkflowSkillTests(unittest.TestCase):
    ...
    def test_global_entry_required_terms_match_compact_contract(self):
        ...
        self.assertIn("Haiku-low first", required)
        self.assertIn("before skills/memory/files", required)
        self.assertIn("`fable|max`", required)
        ...
        self.assertIn("ending_verification_plan.py", required)
        self.assertIn("all checks must PASS", required)
        self.assertIn("FAIL creates Fix Task with exact error then fresh End Task", required)
    ...
    def test_executor_skills_support_inline_and_admitted_modes(self):
        ...
        self.assertIn("A low-risk, low-ambiguity text/code edit scoring `0-24` tries the catalog priority producer first", code_text)
        self.assertIn("other eligible implementations execute the Obsidian-context catalog-derived quality pair", code_text)
