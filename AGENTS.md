...
- `python3 tests/skill_behavior_suite.py [case-id ...] [--repeat N] [--rounds N] [--workers N]` — behavioural suite: one real headless Claude session per rule, asserted against the transcript, the files it produced, and the memory events it recorded. `--repeat N` runs each rule N times and reports PASS / FLAKY / FAIL, because a rule that fires sometimes is not a rule. `--rounds N` (default 3) gives a rule that missed up to N clean-sweep attempts — 40 rules × 5 runs is 200 independent sessions, so demanding one flawless sweep would block on tail noise forever, while a genuinely broken rule still never manages a clean round. A session that never executed (rate limit, dropped connection) is reported as unusable, never as a behaviour miss. Each session gets its own disposable clone of the vault (the production vault is never a test target) and the suite installs the source contract into the deployed mirror first, so the children exercise the text under test. Fixtures live outside the repo so they resolve as their own project root.
