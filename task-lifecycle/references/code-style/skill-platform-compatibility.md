...
- Run the checker on changed functional files and include results in project completion flow. From the `~/.claude` directory, use the launcher appropriate to the host:

```sh
# macOS/Linux
python3 skills/task-lifecycle/scripts/skill_platform_check.py check --skills-root skills --baseline skills/task-lifecycle/assets/skill-platform-baseline.json
```

```powershell
# Windows PowerShell
py -3 skills/task-lifecycle/scripts/skill_platform_check.py check --skills-root skills --baseline skills/task-lifecycle/assets/skill-platform-baseline.json
```
