...
Use `execution_domain=python` in routing evidence for Python work. This domain shares the `code-style` reference set with the other code domains but is not interchangeable with their language rules or evidence.
...
- When independent repeated work is present or discovered during optimization, also read `references/code-style/parallelization.md` and parallelize only when the same observable result can be verified.
...
- Present `CODE READY` with Quick Check evidence. A low-risk single-result small task records `intentionally_skipped_simple_task` and returns without an Ending. All other code work is Ending-required: emit `ending-required`, then create and acknowledge exactly one independent verification Agent before finalizing. Multiple unit, integration/API, and runtime checks stay inside that one Ending and retain their own scores. Every required check must PASS.
- Before durable result-memory closeout, compare the active Skill/AGENTS process contract, fresh real execution evidence, and effective project-result memory. Only a memory-only defect permits an append-only correction or projection reconciliation inside Ending. Skill or execution defects write no result memory and return exact evidence to the immutable origin producer for repair and a fresh Ending. The next task reads only effective project-result memory and hides superseded records from active guidance.
- A failing verifier records the exact command/output/error and acceptance gap, then returns the generated repair prompt to the immutable origin producer. That origin repairs only the authorized result, runs Quick Check, presents a new result, and starts a fresh Ending that reruns the original check linked to the failed parent verification; continue for at most three attempts. A missing origin producer or failed repair handoff is BLOCKED, and BLOCKED is not verified.
