---
name: task-lifecycle
description: Qin's mandatory task lifecycle — launch at the start of EVERY task (coding, debugging, refactoring, testing, writing, automation, research) in any project. Never hands the work back: no clarifying questions, no option menus, no mid-task stalls — assumptions are stated and every doubt is raised in the final report. Connects the Obsidian vault, plans and scores difficulty, announces difficulty (band + 0–100 score)/model/steps in a standalone message, enforces Qin's writing gate plus code and prompt style rules, delivers the result first with a bounded producer Quick Check, then runs REAL verification — required by default, one pass carrying the whole check list — in a fix loop until PASS (status vocabulary MAIN RESULT READY / PASS / FAIL / BLOCKED), applies code/process optimization, and records a complete change memory to Obsidian (never as files inside the project repo).
---
...
- **End-Task vocabulary bridge** — the synced style files keep upstream Ending vocabulary; map it to this section: `CODE READY` = `MAIN RESULT READY`; "Ending Real", "End Task-{…}", and "detached background Agent" = this section's ONE independent verification, run to completion inside the current task — never detached, never returning before every check PASSes ("return without polling/waiting" does not apply); `ending-required` = this section's default (verification required, run it now); `intentionally_skipped_simple_task` = the stated low-risk single-result exemption; "immutable origin session/producer" = the producer that owns the repair; "three attempts then BLOCKED" = the ~3-fail change-of-approach rule above; `--producer-receipt` and the GPT model pairs in `spark-small-code.md` have no Claude equivalent — that file is kept byte-identical to upstream for sync fidelity only, its ladder is listed under `retired_architectures`, and the announce's `<score>/100` is a display-only echo, not that ladder's routing.
