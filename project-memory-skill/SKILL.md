...
- Before every eligible producer route, read only the relevant project-scoped broad `Claude Model Switch.md` records and use them for recommendation, including cold start. The producer receipt carries a sanitized learning context; after presentation, bind it to the Ending lifecycle so the terminal Real verdict records the new entry automatically.
...
```bash
python3 ~/.claude/skills/project-memory-skill/scripts/obsidian_model_memory.py recommend --project-root <root> --task-type <type> --module <module> --file <relative-file> --symbol <method-or-symbol> --code-kind <kind> --operation <operation> --complexity <easy|complex>
