...
def _ensure_bundled_python_runtime(required_modules):
    ...
    # Prefer a Claude Code-native bundled runtime; fall back to a co-installed
    # Codex bundled runtime (same dependency layout) when that is what is
    # actually present on this machine.
    candidate_runtimes = (
        Path.home() / ".cache/claude-runtimes/claude-code-runtime/dependencies/python/bin/python3",
        Path.home() / ".cache/codex-runtimes/codex-primary-runtime/dependencies/python/bin/python3",
    )
    current_executable = Path(sys.executable).resolve()
    for bundled_python in candidate_runtimes:
        if bundled_python.is_file() and bundled_python.resolve() != current_executable:
...
def _full_width_artifact_images(path, width_limit, height_limit):
    ...
    for top in range(0, image_height, slice_height):
        ...
        flowable.hAlign = "LEFT"
        flowable._claude_image_buffer = image_buffer
