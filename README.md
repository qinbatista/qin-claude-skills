...
- **RETIRED** — the idea must be provably absent from the *active contract* files (`SKILL.md`, the entry-rule asset, both READMEs). `references/code-style/spark-small-code.md` is deliberately excluded: it is kept byte-identical to upstream for sync fidelity, GPT model names and all, and the section 5 vocabulary bridge is what neutralizes it.
