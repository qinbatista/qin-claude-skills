...
REQUIRED_MATRIX = ["# Adaptive And Admitted Routing Matrix", "Eligible ordinary text/code production enters the compact adaptive runner exactly once", "Two or three independent read-only sources first cost-admit one contextual producer versus a graph", "`workflow-skill` executes only a positively admitted multi-node or locked route", "ordinary-production", "independent-small-sources", "independent-large-sources", "dependent-multi-file", "explicit-routing-no-graph", "explicit-benchmark", "admitted-complex", "One receipt-valid Real PASS retains the selected pair", "two matched PASS outcomes try one lower rung", "quality failure upgrades one rung", "priority producer is schedule-source-only", "Direct uses the exact raw prompt", "public two-world comparison", "dispatcher/controller is disclosed but excluded", "--direct-task", "--bootstrap-task"]
REQUIRED_CODE = ["Obsidian-context catalog-derived quality pair", "Obsidian broad `Claude Model Switch.md`", "project/task/module/file/symbol/code", "One Real PASS retains the pair", "two PASS results downgrade one rung", "quality FAIL upgrades one rung", "priority producer is reserved for fixed disjoint-source schedule branches", "zero-result operational failure", "Old local model capability sources remain legacy read-only", "mandatory post-result Ending lifecycle still runs", "When thread tools are callable", "When thread tools are unavailable", "outer host immediately", "Never inspect app-server internals", "same-task Ending subagent", "different `verify-skill` worker performs independent verification"]
REQUIRED_VERIFY = ["Verification has two scopes", "Ending handoff audit", "Every non-Ending user task creates that separate persistent Agent thread", "Never add Mini/Fast Verify before the user's first presentation", "current user-selected model", "PASS or explicit BLOCKED", "no extra tests, APIs, broad regression, user questions, waits, or automatic repair", "new explicit user task"]
...
REQUIRED_MANAGEMENT = ["Do not load this skill for ordinary exact-scoped read-only work or Direct/Global benchmark worker arms", "directly only for an explicit routing-record", "admitted a delegated route", "broad Claude Model Switch.md", "project/task/module/file/symbol/code", "obsidian_model_memory.py", "Ending Real starts with the producer receipt", "terminal ledger event records the result automatically", "legacy read-only", "Never push/sync/publish unless the user explicitly requested publishing"]
REQUIRED_ENTRY = ["# Task Lifecycle", "Merge this section into `~/.claude/CLAUDE.md`", "pipe exact user text once via `/usr/bin/python3", "non-TTY", "obsidian_adaptive_model_runner.py", "before skills/memory/files", "except one-source exact read-only", "Wait 60s", "session id:resume once <=60s", "return result/pending", "NEVER spawn/read", "Receipt nodes direct", "never re-enter runner", "Saved ladder", "model-update refreshes cache", "never fetch", "Missing Obsidian:saved cold start,no block", "Auto=saved pair", "2 Real PASS:down 1 rung", "quality FAIL:up 1", "zero-result:stronger fallback", "priority producer:schedule sources only", "2-3 sources cost-admit before reads", "single or fused graph", "dependent multi-file:one producer", "Producer owns files/skills/Mini Test", "light smoke", "heavy/API/large/side-effect checks syntax+names/references", "create/link `End Task-{task name}` if available", "never subtask/emulate/wait/self-verify", "Ending <=60s evidence-only", "never gates", "Benchmark 3 tiers", "current entry model", "Direct fixed/no verify", "Auto receipt=child/graph", "task vs task+Ending", "controller excluded", "Complex stage pairs", "Exact one-source read-only:one bounded rg/file", "no reread/full read/precheck", "No hook", "Final PASS/BLOCKED Ending-only"]
...
def validate(skill_dir):
    ...
    if entry_body != texts["global_agents"]:
        failures.append("global entry asset does not exactly match global CLAUDE after removing its merge directive")
    live_text = "\n".join(texts.values())
