...
| ordinary-code-domain | Implement or edit an active registry-owned code domain. | adaptive producer -> code-skill -> Quick Check -> result | Small low-risk edit tries `haiku`; otherwise contextual quality pair. | Present changed paths, behavior, and Quick Check. | Separate scored real-test tasks; all checks must PASS. |
...
- `haiku` is first priority for eligible small low-risk edits and remains available for admitted schedule-source branches; a matching quality failure suppresses it and upgrades the next task.
